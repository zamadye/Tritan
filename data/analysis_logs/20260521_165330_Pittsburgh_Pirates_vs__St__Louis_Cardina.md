# Market Analysis — 2026-05-21 16:53 UTC

## Market
- **Question:** Pittsburgh Pirates vs. St. Louis Cardinals
- **Category:** sports
- **YES Price:** 52.0%
- **Days Left:** 7
- **Volume 24h:** $174,788
- **Liquidity:** $460,386

## Statistical Prior
- **p_base:** 53.0%
- **Source:** logistic:sports
- **N (historical):** 6077
- **Raw gap:** 1.0%

## News Sources Fetched
### Macro Context
- Fear&Greed: 29/100 (Fear) | trend: improving
- BTC: $76,972 (-0.3% 24h) → neutral
- Fear&Greed: 29/100 (Fear) | 7d trend: improving
- BTC: $76,956 (-0.3% 24h) → neutral
- Fed market expectations:

### General
- [FEAR&GREED 7d trend] 29(Fear) → 27(Fear) → 25(Extr) → 28(Fear) | sentiment improving
- Will the Fed increase interest rates by 50+ bps after the Ju → YES=50%
- [SPORTS] Recent games:
- PIT @ STL | Scheduled | St. Louis Cardinals(home) 28-20 score:0 vs Pittsburgh Pirates(away) 25-24 score:0
- [SPORTS] Injuries:
- ?: Derek Law — Day-To-Day (Surgery)
- ?: Tommy Henry — Day-To-Day (Surgery)
- ?: Spencer Giesting — Day-To-Day (Not Specified)

### Multi-Source
- [MULTI-SOURCE NEWS] 2 sources for: pittsburgh pirates louis cardinals
- [Bing News]
- [Thu, 21 May 2026] pittsburgh pirates louis cardinals
- → The St. Louis Cardinals look to win the series at home against the Pittsburgh Pirates on Thursday afternoon. The Cardinals won Game 1, 9-6, while th
- [Thu, 21 May 2026] Pirates vs. Cardinals prediction, odds, time: 2026 MLB picks for Thursday, May 21 from proven model
- → Here's how to watch Thursday's St. Louis Cardinals vs Pittsburgh Pirates game, including start times, TV channels, scores and how to stream.
- [Thu, 21 May 2026] Where to watch Pittsburgh Pirates vs St. Louis Cardinals: TV channel, start time, streaming for May 21
- → The Pittsburgh Pirates (24-24) are on their way to Busch Stadium on Thursday where they will attempt to beat the St. Louis Cardinals (28-19). The li

## LLM Decision
- **Action:** SKIP
- **Confidence:** 65%
- **p_true:** 44.2%
- **Information Gap:** False
- **Crowd Bias:** False
- **Catalyst Type:** STATISTICAL_EDGE
- **Catalyst Strength:** 0.72

### Reasoning
Strong statistical edge: markets priced at 52% YES resolve YES only 44.7% of the time (n=13,967), indicating systematic YES overpricing of ~7.3%. The YES price (52%) is above the historical resolution rate (44.7%), creating a ~5.4% edge on NO. Additionally, Pirates have significant injuries (Jared Jones SP 60-Day IL, Ryan O'Hearn RF 10-Day IL, Joey Bart C 10-Day IL) which further weakens the YES case. Past decisions on this exact market were SKIP due to gap<5% gate, but the stat signal alone provides actionable edge. Sports pattern shows WR=73% from 33 trades supporting this systematic approach.

## Gate Result
- **Passed:** ❌ NO
- **Reason:** no stat edge gap=1%<5%

## Tool Calls
- `get_sports_data({'sport': 'mlb', 'team': 'Pittsburgh Pirates'})` → INJURY [pittsburgh pirates] Jared Jones (SP): 60-Day-IL
INJURY [pittsburgh pirates] Ryan O'Hearn (RF
- `get_odds({'sport': 'baseball_mlb', 'team': 'Cardinals'})` → Odds API error: 401 {"message":"Usage quota has been reached. See usage plans at https://the-odds-ap
- `search_web({'query': 'Pittsburgh Pirates vs St. Louis Cardinals upcoming game 2025'})` → [Wed, 27 Aug 2025] Google News
[Wed, 27 Aug 2025] Pittsburgh Pirates at St. Louis Cardinals Game Sto
- `search_web({'query': 'Pittsburgh Pirates Cardinals series results August 2025'})` → [Wed, 27 Aug 2025] Google News
[Mon, 25 Aug 2025] Pirates 2-1 Cardinals (Aug 27, 2025) Final Score -
- `get_market_context({'keyword': 'Pittsburgh Pirates'})` → No related markets found for 'Pittsburgh Pirates'
- `search_web({'query': 'Cardinals vs Pirates August 28 2025 odds pitching matchup'})` → [Wed, 20 May 2026] Google News
[Mon, 27 Apr 2026] Cardinals vs Pirates Prediction, Odds, Moneyline, 
- `search_web({'query': 'MLB standings 2025 August Pirates Cardinals'})` → [Mon, 18 May 2026] Google News
[Tue, 19 May 2026] 2026 MLB Standings and Records: Regular Season - M

## Outcome
_Pending — will be updated when market resolves_

---
*Mode: LIVE | DRY_RUN: true*