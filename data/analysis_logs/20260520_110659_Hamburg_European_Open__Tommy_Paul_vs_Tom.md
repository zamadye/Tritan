# Market Analysis — 2026-05-20 11:06 UTC

## Market
- **Question:** Hamburg European Open: Tommy Paul vs Tomas Etcheverry
- **Category:** sports
- **YES Price:** 57.0%
- **Days Left:** 6
- **Volume 24h:** $704,214
- **Liquidity:** $90,196

## Statistical Prior
- **p_base:** 60.4%
- **Source:** logistic:sports
- **N (historical):** 6077
- **Raw gap:** 3.4%

## News Sources Fetched
### Macro Context
- Fear&Greed: 27/100 (Fear) | trend: deteriorating
- BTC: $77,443 (+0.6% 24h) → neutral
- Fear&Greed: 27/100 (Fear) | 7d trend: deteriorating
- BTC: $77,421 (+0.6% 24h) → neutral
- Fed market expectations:

### General
- [FEAR&GREED 7d trend] 27(Fear) → 25(Extr) → 28(Fear) → 27(Fear) | sentiment worsening
- MicroStrategy sells any Bitcoin by May 31, 2026? → YES=50%
- [CAUSAL CHAINS] Relevant cause-effect patterns:
- → ECB rate cut → EUR weakens vs USD → EUR/USD falls
- → EU GDP beats expectations → EUR strengthens
- → EU political crisis (elections) → EUR uncertainty → falls
- → EUR/USD above 1.10 → European exports competitive → EU stocks rally

### Multi-Source
- [MULTI-SOURCE NEWS] 2 sources for: hamburg european open: tommy paul
- [Bing News]
- [Fri, 15 May 2026] hamburg european open: tommy paul
- → Hamburg Open 2026 (ATP 500): stacked clay-field before Roland Garros. Defending champ Flavio Cobolli, Zverev, Auger-Aliassime, Shelton, Fonseca, Men
- [] Hamburg Open 2026: Results, Draw, Schedule, Entry List, Prize Money and Predictions
- [Metaculus]
- [] Will a case of Bundibugyo Ebola disease be first confirmed in the US b
- → Community: pending

## LLM Decision
- **Action:** SKIP
- **Confidence:** 65%
- **p_true:** 60.4%
- **Information Gap:** False
- **Crowd Bias:** False
- **Catalyst Type:** NONE
- **Catalyst Strength:** 0.10

### Reasoning
Stat signal (n=13,967) suggests YES at 57% is overpriced (historically resolves 44.7%), but gap (3%) falls below 5% gate threshold. No current injury reports, form data, or recent match results available for either player. No related markets to cross-verify. Previous analyses on same market also recommended SKIP. Entry 50-60% historically weak (WR=33%).

## Gate Result
- **Passed:** ❌ NO
- **Reason:** no stat edge gap=3%<5%

## Tool Calls
- `get_sports_data({'sport': 'tennis', 'team': 'Tommy Paul'})` → No injury data found for tennis Tommy Paul
- `get_sports_data({'sport': 'tennis', 'team': 'Tomas Etcheverry'})` → No injury data found for tennis Tomas Etcheverry
- `get_market_context({'keyword': 'Hamburg European Open tennis'})` → No related markets found for 'Hamburg European Open tennis'
- `search_web({'query': 'Tommy Paul Hamburg European Open 2025 results form'})` → [Mon, 04 May 2026] Google News
[Fri, 24 Oct 2025] Tennis 2026 schedule, Grand Slam dates and tournam
- `search_web({'query': 'Tomas Etcheverry Hamburg European Open 2025 results form'})` → [Mon, 04 May 2026] Google News
[Sun, 20 Jul 2025] Tennis 2026 schedule, Grand Slam dates and tournam
- `search_web({'query': 'Tommy Paul tennis 2025 recent results form clay court'})` → [Thu, 21 Aug 2025] Google News
[Sat, 24 May 2025] US Open 2025: Laver Cup Form Guide - Laver Cup
[Tu
- `search_web({'query': 'Tomas Etcheverry tennis 2025 recent results form clay'})` → [Mon, 18 May 2026] Google News
[Tue, 07 Apr 2026] ATP Hamburg Day 3 Predictions Including Felix Auge
- `get_odds({'sport': 'tennis_atp', 'team': 'Tommy Paul'})` → Odds API error: 404 {"message":"Unknown sport","error_code":"UNKNOWN_SPORT","details_url":"https://t
- `search_web({'query': '"Tommy Paul" vs "Tomas Etcheverry" head to head 2025'})` → [Thu, 14 Aug 2025] Google News
[Sat, 17 May 2025] Jannik Sinner vs. Felix Auger-Aliassime Cincinnati
- `search_web({'query': 'Hamburg European Open 2025 draw results Tommy Paul Etcheverry'})` → [Mon, 18 May 2026] Google News
[Fri, 19 Jul 2019] ATP Hamburg Day 3 Predictions Including Felix Auge

## Outcome
_Pending — will be updated when market resolves_

---
*Mode: LIVE | DRY_RUN: true*